...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::prefix('panier')->middleware('role:client')->group(function () {
        ...
        Route::patch('/items/{item}',   [PanierController::class, 'updateQuantity']); // Added missing update quantity
    ...
    Route::prefix('cooperative')->middleware('role:artisan')->group(function () {
        ...
        Route::post('/update',        [CooperativeController::class, 'update']); // Used POST for image/file uploads
        Route::get('/commandes',      [CommandController::class, 'artisanOrders']); // Added order management for artisans
    });

    // ── Product Management (Artisan) ─────────────────────────────────────────
    Route::middleware('role:artisan')->group(function () {
        Route::post('/products',                     [ProductController::class, 'store']);
        Route::post('/products/{product}',           [ProductController::class, 'update']); // POST for multipart image updates
    ...
    Route::prefix('admin')->middleware('role:admin')->group(function () {
        ...
        Route::patch('/commandes/{command}/status',         [CommandController::class, 'updateStatus']);
